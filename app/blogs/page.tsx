...
export default function BlogPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (heroBadgeRef.current) gsap.set(heroBadgeRef.current, { opacity: 1, force3D: false });
      if (heroTitleRef.current) gsap.set(heroTitleRef.current, { opacity: 1, force3D: false });
      if (heroDescRef.current) gsap.set(heroDescRef.current, { opacity: 1, force3D: false });
  ...
  return (
    <main className="min-h-screen bg-transparent text-foreground relative z-10">
      {/* Hero Section */}
      <section ref={heroSectionRef} className="relative h-[60vh] sm:h-[70vh] lg:h-[100vh] w-full flex items-end justify-center text-center" style={{ isolation: 'isolate', contain: 'layout style paint' }}>
        <video
          ref={videoRef}
